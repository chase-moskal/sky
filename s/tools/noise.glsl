...
float whiteNoise(float resolution, float offset){
	return rand((floor(gl_FragCoord.xy * resolution) + offset) * mod(time / 10000.0, 1.0));
}

float makeStarfield(vec3 direction, float frequency, float intensity){
